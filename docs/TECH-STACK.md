...
| **代价** | Helios 的几何光线追踪在"单面不遮挡 / 双面重复计数"上存在已知局限，导致**判据 C1 目前无法在该模型下检验**（见 `TASKS.md` T-04）。 |
| **Helios 的新定位** | 阶段一计算模型（二维剖面 + **逐层辐射输出**，T-03 已验证 W1–W6）；**不承担**行向对比（C1）的验证 —— 该验证待判据复核或三维条带几何实现。RATP 保留为已记录备选（许可更宽松、纯 CPU，但需源码构建与 MinGW 运行库）。 |
| **回退** | 若判据 C1 经复核确认必须用湍流介质模型 → 需先解决 Qt 依赖（装 Qt 绑定，或绕过 `grid`/`runratp` 直接用 F2PY 层）；届时本条改为「已回退」并重跑 C1 回归。 |

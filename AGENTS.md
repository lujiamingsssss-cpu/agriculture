...
**最终目标**：输入地块与农艺参数，输出推荐带型（行比 / 带宽 / 行向）、光截获分布可视化、土地当量比（LER）。

**当前阶段：阶段一 · Demo —— 只做上面这个目标的一半**：证明光在两作物间的分配
**可算、可看、可比较**（时间 / 行向 / 地点 / 日期 → 光斑 + 日变化曲线）。
🔴 **"给出推荐带型"归阶段二**，依赖重做冠层表达；阶段一**不得宣称**已具备推荐能力。

阶段划分与判据见 [`docs/BASELINE.md`](docs/BASELINE.md)；计划原点与路线见 [`docs/TASKS.md`](docs/TASKS.md) §1–§2。
